...
if [ -d "$PAM_COPY_DIR" ]; then
	echo "$PAM_COPY_DIR already exists."
else
	mkdir "$PAM_COPY_DIR"
	echo "$PAM_COPY_DIR not found. Directory created."
fi
...
if grep -q "pam_pwquality.so" "$PAM_COPY_ALTERED"; then
	echo "Copied file successfully changed in place using sed. Verify changes before altering original file."
	...
	echo "An error occured while trying to use sed. Requires manual change. Changes appended to bottom."
